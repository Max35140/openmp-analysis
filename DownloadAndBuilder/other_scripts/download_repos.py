...
SCRIPT_PATH = BASE_PATH+r"\scripts\CI"
...
def cloneRepo(repoUrl, path, commit_hash=None):
    try:
        ...
        if commit_hash is None or current_hash == commit_hash:
            ...
            subprocess.check_output(f'git checkout {commit_hash}', cwd=path, stderr=subprocess.STDOUT, shell=True,
                                    encoding='UTF-8')
            return commit_hash
    except subprocess.CalledProcessError as e:
        print(f"ERROR: downloading Repo ({repoUrl}) to ({path}):")
        print(e.output)
